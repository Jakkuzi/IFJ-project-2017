...
//                actualFunction->paramCount += 1;//todo: takhle ne
	...
	//	tmp = tmp->next;
		...
		if(actualFunction == NULL)
		    return 3;
	        else if(actualFunction->defined != 1)
		    return 3;
	...
	//TODO: ostatni pripady... else if...
		...
		return 3;
	    else if(actualFunction->itemType == item_type_function && actualFunction->varData == NULL){
		if(actualFunction->funcData == NULL)
		    return 3;
		else{
		    if(actualFunction->funcData->returnType != idReturnType)
			return 4;
		    else{
			BTNodePtr subBTree = actualFunction->funcData->ParamRootPtr;	//asi spatne
	//TODO: dodelat
			
		    }
		}
	    }
	    else if(actualFunction->itemType == item_type_variable && actualFunction->funcData == NULL){
		if(actualFunction->varData == NULL)
		    return 3;
		else{
		    if(actualFunction->varData->type != idReturnType)
			return 4;
	/*TODO: vlozeni do stromu?
		    else */
		}
...
/*	else if(id == Lenght){
		...
		return 3;
	    else if(actualFunction->varData->varDataType != var_string)
		return 4;
	}
	else if(id == Substr){
	    tmp = tmp->next->next;
		...
		return 3;
	    else if(actualFunction->varData->varDataType != var_integer)
		return 4;
	}*/
